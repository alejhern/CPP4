...
int	main(void)
{
	...
	meta->make_sound();
	std::cout << std::endl;
	dog_brain = dog->get_brain();
	cat_brain = cat->get_brain();
	...
	another_dog = new Dog();
	another_dog_brain = another_dog->get_brain();
	...
	*another_dog_brain = *dog_brain; // Using Brain's assignment operator
	...
	std::cout << std::endl;
	Animal *c_dog(new Animal(*dog));
	c_dog_brain = c_dog->get_brain();
